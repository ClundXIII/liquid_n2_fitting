...
const bdt T_t  = 63.14;     //Tripelpunk T
const bdt d_H  = 5590;      //Verdampfungsenthalpie
...
vector<bdt> run_sim(bdt p_0, bdt n_fl, bdt dt, bdt stop, vector<bdt> p_mess, bool print, ostream& print_s, bdt offset){
    vector<bdt> data;
    data.push_back(p_0);
    data.push_back(n_fl);
    data.push_back(1 /( ( ((-R_m /d_H)*log(data[0]/p_t)) + (1./T_t) ) ));

    cout << "start values: p=" << data.at(0) << ", n_fl=" << data.at(1) << ", T=" << data.at(2) << ", P_const=" << P_const << endl;
    ...
    for (; t<=stop; t+=dt){
        ...
        deltaInsg += data.at(0)-p_mess.at(it_pos);
            if (print) print_s<<t+offset<<" "<<data.at(0)/100<<" "<<data.at(1)<<" "<<data.at(2)<<endl;
        if (it_pos<(p_mess.size()-1))
            it_pos++;
        else
            {}//cout << "p_mess out of range" << endl;
...
bdt optimize_P(bdt stepsize, bdt p_0, bdt stop, vector<bdt> vec_p, bdt n_fl, bdt P_min, bdt P_max, bdt delta_p_epsilon){
    ...
    do {
        ...
        vector<bdt> retData = run_sim(p_0, n_fl, stepsize, stop, vec_p, false, cout, 0);
        //cout << "run with end p=" << retData[0] << " and average sqrt(sum(delta p)^2) of " << pow(retData[3], 0.5) << ", ended at t=" << retData[4] << endl;
        //cout << "needed p_end=" << p_end << ", deltap=" << (retData[0]-p_end) << endl;
    ...
    cout << "found P_const=" << P_const << endl;
...
void optimize(bdt stepsize, vector<bdt> vec_p, bdt n_min, bdt n_max, bdt P_min, bdt P_max, bdt p_end_epsilon, bdt sum_delta_p_epsilon, bdt offset){
    ...
    do {
        ...
        cout << "fit had sum_delta_p of " << sum_dp << endl;
        if (sum_dp<0)
            n_min = (n_min+n_max)/2;
        else
            n_max = (n_min+n_max)/2;
    ...
    cout << "plotting latest fit ..." << endl;
    ofstream fout("fit_plot.txt");
...
int main(int argc, char* argv[]){
    ...
    if (argc == 1){
        ...
        for (bdt t=0; t<stop; t+=dt){
            cout<<t<<" "<<data.at(0)/100<<" "<<data.at(1)<<" "<<data.at(2)<<endl;
            data = RK4_schritt(data, dt);
        }
    }
    else if(argc == 2){
        ...
        if (argument == "plot"){
            cout << "Filename>";
            char inputName[256];
            cin.get(inputName, sizeof(inputName));
            ...
            cout << "read " << file_t.size() << " datasets." << endl;

            dt = file_t.back()/file_t.size();

            cout << "Stepsize (detected:'" << dt << "'):>"; cout.flush();
            string tempS;
            cin >> tempS;
